...
///////////////////////////////////////Physics 3d
Physics3D::Physics3D(Entity381* newEnt):Aspect(newEnt){
}
...
void Physics3D::Tick(float dt){
	//Update altitude (limited by climbing rate, slow increase);
	if(entity->desiredAltitude> entity->altitude){
			entity->altitude += entity->climbingRate * dt;
		}
		// Decelerating
	...
	// Calculate x and z and y coordinates from sin and cos
	entity->velocity.z = Ogre::Math::Sin(Ogre::Degree(entity->heading)) * entity->speed;
	entity->velocity.x = Ogre::Math::Cos(Ogre::Degree(entity->heading)) * entity->speed;
	...
	//std::cout << entity->altitude <<std::endl;
